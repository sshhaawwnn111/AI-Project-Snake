...
class Agent:
    ...
    def __init__(self, model_path, snake_num=1):
...
def test():
    ...
    record = 0
    record2 = 0
    agent1 = Agent('model/agent_5000ep.pth', snake_num = 1)
    agent2 = Agent('model/model.pth',snake_num = 2)
    game = snake_game()
    while True:
        # get old state
        state_old = agent1.get_state(game)
        ...
        if done:
            ...
            print('Game', agent1.n_games, 'Score', game.score1, 'Record:', record)
            print('Game', agent2.n_games, 'Score', game.score2, 'Record:', record2)

            plot(record/agent1.n_games, record2/agent2.n_games)
